<template>
  <div id="ChangeName">
    <Header :showBack="true">
      <span slot="title">修改微信号</span>
    </Header>
    <section class="content-box">
      <div class="change-box">
        <cube-input v-model="value" placeholder="请输入微信号" autocomplete clearable></cube-input>
      </div>
      <cube-button @click="changeWechat">保存</cube-button>
    </section>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      changeWechat: function () {
        ...
        // 提交数据
        this.$store.dispatch('editWeChat', {wx_account: this.value}).then(function (res) {
          if (res.code == 0) {
            _this.$createToast({
              time: 2000,
              txt: '修改成功',
