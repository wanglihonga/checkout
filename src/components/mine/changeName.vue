<template>
  <div id="ChangeName">
    <Header :showBack="true">
      <span slot="title">修改姓名</span>
    </Header>
    <section class="content-box">
      <div class="change-box">
        <cube-input v-model="value" placeholder="请输入姓名" autocomplete clearable></cube-input>
      </div>
  ...
  export default {
    ...
    methods: {
      changeName: function () {
        ...
        // 提交数据
        this.$store.dispatch('editName', {name: this.value}).then(function (res) {
          if (res.code == 0) {
            _this.$createToast({
              time: 2000,
              txt: '修改成功',
