<template>
  <div class="item3" @click="goDetail(innerData.activieyId)">
    ...
    <section class="txt-box">
      ...
      <ul>
        <li><label>活动时间: {{innerData.startTime}}</label></li>
        <li>{{innerData.address}}</li>
      </ul>
    </section>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      goDetail: function (activityId) {
        ...
        // 判断是否有权限
        if (this.flag == 1) {
          this.$router.push({path: '/activityDetail', query: {activityId: activityId, page: 'myCenter'}})
          return
        }
